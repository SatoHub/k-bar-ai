...
def main():
    ...
    print(
        f"荒れ度: {up.get('level')} (波乱{round((up.get('expected_upset_rate') or 0) * 100)}%) "
        f"→ 狙い自動切替"
    )
    ...
    def show(title, res):
        ...
        for s in res["suggestions"]:
            ...
            stake = (
                f"{s['stake_min']}円"
                if s["stake_min"] == s["stake_max"]
                else f"{s['stake_min']}〜{s['stake_max']}円"
            )
            gami = "ガミ無" if s["gami_free"] else "ガミ有"
            drop = f" / ガミ除外{s['dropped_points']}点" if s["dropped_points"] else ""
            print(
                f"{mark}【{BET_JA.get(s['bet_type'], s['bet_type'])}】{s['rationale']}"
            )
            print(
                f"          {s['points']}点 (1点{stake}) = 計{s['cost']}円 [{gami}]{drop}"
            ...
            if s["axis"]:
                names = "・".join(
                    post_to_name.get(h, str(h)) for h in (s["axis"] or [])[:1]
                )
                print(
                    f"          軸: {names} / 相手: "
    ...
    show(
        "おまかせ × 均等買い", suggest(BUDGET, horses, ranked, level, alloc_mode="flat")
    )
    show(
        "券種手動: 三連複+三連単+ワイド(予算指定)",
        suggest(
            BUDGET,
            horses,
            ranked,
