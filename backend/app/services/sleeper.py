...
def compute_sleeper(
    ...
    if before_date is not None:
        runs = [
            r for r in runs if (d := _parse_date(r.get("date", ""))) and d < before_date
        ]
    ...
    same = [
        r
        for r in runs
        if r.get("surface") == today_surface and r.get("finish_position")
    ]
    n = len(same)
    if n == 0:
    ...
    # 今走馬場での実力
    ability = min(
        1.0, place_rate + (0.2 if has_win else 0.0) + (0.25 if graded_good else 0.0)
    )
    ...
    score = 0.0
    primary = (
        surface_mismatch and recent_poor and unpopular and n >= 2 and ability >= 0.3
    )
    if primary:
