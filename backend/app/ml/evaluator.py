...
def evaluate_model(version: str) -> str:
    ...
    lines = [
        ...
        "",
        "--- Training Metrics ---",
        f"  Accuracy:       {train_metrics.get('accuracy', 'N/A'):.4f}" if isinstance(train_metrics.get('accuracy'), float) else "  Accuracy:       N/A",
        f"  F1 Score:       {train_metrics.get('f1', 'N/A'):.4f}" if isinstance(train_metrics.get('f1'), float) else "  F1 Score:       N/A",
        f"  ROC AUC:        {train_metrics.get('roc_auc', 'N/A'):.4f}" if isinstance(train_metrics.get('roc_auc'), float) else "  ROC AUC:        N/A",
