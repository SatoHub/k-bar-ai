...
**最終更新:** 2026-06-14（ガミ防止UI＋記録馬券の削除/枠色表示＋馬券の当落自動判定を実装・本番反映）

---

## 🔵 次回開始ガイド（RESUME HERE — まずここを読む）

### ★2026-06-14セッションの成果（全push済み・最新コミット `d38d601`・本番実証済み）
**ユーザーの馬券運用支援（ガミ防止／収支の自動精算）を実装。3コミットとも本番デプロイ成功・実機確認済み。**

- ✅ **ガミ防止UI**(`119743c`): 馬券シミュレーター(`/races/{id}/simulate`右カラム)に3点追加。
  ①複数買いの買い目テーブルに「ガミ防止ライン(必要最低オッズ=総投資÷1点掛け金)」バナー
  ②当たっても総投資割れする買い目に⚠️ガミバッジ＋行を赤表示 ③合計サマリーに全体ガミ警告。
  ①②は**複数買い(ボックス/F/流し)かつオッズ取得済み**のときのみ表示／③は構成全体で投資割れ時
- ✅ **記録済み馬券の改善**(`119743c`): 削除ボタン(`DELETE /bets/{id}`は既存・フロントに`deleteBet`追加)＋
  馬名テキスト→**JRA枠色付き馬番バッジ**表示(`RecordedBetBadges`・着順あり券種は→で連結)
- ✅ **馬券の当落・払戻を自動判定**(`d38d601`)★ユーザー要望: **これまで収支サマリーが「当たっても回収率0%」**
  だった原因=作成時に`actual_payout`/`is_hit`未設定＆自動精算が無かった。`settle_bets()`新設＝結果
  (`finish_position`)が出た未判定(is_hit IS NULL)の馬券を全8券種で当落判定、払戻=`floor(掛け金×
  odds_at_bet/100)×100`(シミュ推定と同式・JV-Data非依存)。複勝/ワイドは頭数で対象着順切替(8頭~3着/
  5-7頭~2着)。`get_bet_summary`/`get_bets`先頭で自動実行→**TOP画面を開くだけで反映**。`POST /bets/settle`＋
  収支カード「再判定」ボタンで手動も可。判定ロジックは18ケースで検証。**本番で136件判定・回収率64.6%/
  的中率1.5%(2/136)を実証**(=広く1点ずつ買う今の買い方がそのまま数字化＝ガミ防止UIで絞る動機づけに)
  - ⚠️ベット時オッズ未記録の馬券(主に複勝)は当落判定のみで払戻null。結果未確定レースは据え置き(後で自動判定)

### ★2026-06-13セッションの成果（全push済み・最新コミット `8ebe28c`）
**UI改善・バグ修正・モデル検証・成績ウォッチ基盤の整備を実施。本番はすべて反映済み。**

- ✅ **deploy.yml堅牢化**(`a37439d`): set -euo pipefail＋`git pull --ff-only`＋pull後HEADがgithub.sha一致検証。
  サイレント固着を確実検知。⚠️`script_stop`はappleboy/ssh-action@v1に無い入力(削除済)
- ✅ **オッズキーバグ修正**(`b437984`): netkeibaの組合せキーは区切りなし連結("141516")だが、フロント
  `buildComboKey`とbackend`parse_combo_odds`が"-"区切りで参照しmiss→**複数馬券の組合せオッズが全て「—」**
  だった。3箇所をjoin("")に統一。⚠️**終了レースは組合せオッズがnetkeiba側で消える**(人気上位278/3360のみ残存)＝仕様
- ✅ **シミュレーション画面**: 3カラム横並び(`1fada1d`)/メイン幅1280→1760px(`5cae36b`)/中央カラムを
  券種タブ＋折りたたみでコンパクト化(`13562b9`)
- ✅ **レース詳細AI予測表**(`d4aa682`): 余白解消＋PC版に予想根拠(SHAP説明)インライン表示。SP版はトグルのまま
- ✅ **馬画像修正**(`492566c`): 画像URLが`/img/horse/240_320/{id}.jpg`で常に404→正しい
  `show_photo.php?horse_id=&no=spdb&default_image=netkeiba`に修正(実写真表示)。No Imageプレースホルダーの
  二重エンコードも修正
- 🔴 **結果通知バグ修正**(`5f23528`): **結果通知LINEが30日間ずっと400で全失敗**していた。原因=賭けの無い
  レース(bet=none)で空文字textコンポーネント→LINE Flexが空text非許容。修正＆本番でsuccess実証
- ✅ **AI回収率を新規実装**(`8ebe28c`): 結果＋確定単勝オッズから「AIの◎に毎レース単勝¥100」想定の回収率を
  自動算出しAI成績ページに表示(手入力不要)。実測: 2026-05は79.1%/2026-06は57.4%(市場並み)
- ⏸ **モデル精度向上は構造的に頭打ちと確定**(詳細=`docs/20260613-model-improvement-findings.md`)。
  学習データ161万件(1986-2021)はローカルに有り。相対特徴量・LambdaRank・回収率志向の妙味モデルを
...
4. ✅ **馬券記録の自動精算 → 2026-06-14完了**(`d38d601`)。`settle_bets()`でactual_payout/is_hitを
   自動充填、収支サマリー取得時に自動実行。本番実証済み。残課題=複勝のベット時オッズ未記録分の払戻欠落
5. 🟢 **運用ウォッチ**: 月次でAI回収率・的中率を見て、条件別の偏り(妙味の芽)が出ないか観察＝方針書フェーズ1
6. ⭐🔵 **【ユーザー希望・次回検証】控除率超えの可否を Benter方式バックテストで決着させる**:
   今日の検証は「素朴なEV(p_実力×オッズ-1)」で失敗(docs/20260613-...参照)。**正しい手法は未検証**。手順:
   - ①オッズ非依存の実力モデルで `P_fund`（build_feature_matrix流用・<2020学習/>=2020テスト）
   - ②オッズから市場勝率 `P_market`（レース内で 1/win_odds を正規化しオーバーラウンド除去）
   - ③**メタモデル**: 過去データで `実際の勝敗 ~ logit(P_fund)+logit(P_market)` をロジスティック回帰で学習(重みを最適化)
   - ④合成確率 `P_combined` が `P_market` を上回る馬=妙味。**ケリー基準**で配分し月別ROIをバックテスト
   - ⑤控除率(単勝20%)超えのセグメントが本当に無いか確認。三連単等の高配当券種(歪み大)版も検討
   - ⚠️個人JRAはリベート無し・控除率高め＝超えは難関。効いても薄い見込みだが「公開データにedgeがあるか」を厳密決着
   - 使い捨てスクリプトで実験→効果あれば本番にvalue指標として組込み(AI回収率の隣に「妙味度」等)
7. 🔵 (大きめ・任意) **レバーA**: スクレイパーを脚質(コーナー通過順)・オッズ時系列対応に改修。現在レースでも
   これらを取得できれば新特徴量になり得る(過去データは100%あるが直近スクレイプは0%)。ただし控除率超えは難関
8. 🟢 他画面(ダッシュボード/収支)のUI点検（レース詳細で見つけたような余白/バグの有無）
...
### 2026-06-13 追加分（コミット `b5eb98a`）
- ⏸ **#3 モデル再学習は保留**（ユーザー判断）。理由: JV-Data確定オッズで結合できる学習エントリは
  **321/160万=0.02%**（アプリDBは既にnetkeibaのwin_oddsを99.6%保有）。効果ゼロ。
  本来の効果を出すには**過去数年のJV-Data一括取得**（重い・契約3ヶ月の時計）が前提条件
- ✅ **#4 馬場状態（含水率・クッション値）の本番表示を達成**: JRA公式「馬場情報」スクレイパー新設。
  **血統/確定オッズと違いVPSが自身でJRA公式を直接スクレイプ＝自宅PC同期不要**(設計が別)。
  - scraper `app/scraper/jra_baba.py`(Playwright必須・値はJS注入。`#turf_line/#dirt_line td.gm/.c4`、
    クッション値は正規表現。競馬場はbaba indexから動的discovery)
  - table `track_condition_details`(racecourse_name×measured_date)、racesと(racecourse_name,race_date)で結合。
    alembic `a7b8c9d01234`。store=`store_track_conditions`
  - scheduler `job_track_condition`(id=track_condition・開催日9/12/14時15分・非開催日は自動skip)
  - API `GET /races/{id}/track-condition`、フロント `TrackConditionInfo.tsx`
  - ✅本日(土)の東京/阪神/函館で実値取得(東京:クッション10.2・芝含水率12.3/11.1%・ダート6.7/8.1%)、join検証OK
  - ⚠️ **過去遡及不可**(JRA公式は開催日のみ掲載)。VPSスケジューラが今後の開催日に自動蓄積

### 次回やること（優先度順）★ここから再開
1. ✅ **血統を本番表示**（完了。(a)カバレッジ向上=UMセットアップ時に後日）
