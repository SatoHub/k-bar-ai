...
**最終更新:** 2026-06-13（UI改善多数＋オッズキーバグ修正＋モデル改良検証(頭打ち確定)＋結果通知バグ修正＋AI回収率実装）
...
4. 🟡 **馬券記録の自動精算**(任意): 収支ページの回収率は自動精算が無く手入力依存で常に0。
   結果後にactual_payoutを自動充填する精算jobを足せば、ユーザー自身の馬券P&Lも機能する
