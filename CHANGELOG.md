...
### Added
- **馬券シミュレーター: ボックス・フォーメーション・流し買い対応**
  - `betCalculations.ts` (新規): 組合せ計算の純粋関数（C(n,k), P(n,k), ボックス/フォーメーション/流し）
  - `BettingSimulator.tsx` 大幅拡張:
    - 買い方選択ピル（通常/ボックス/フォーメーション/流し）を馬券種ピルの下に追加
    - ボックス: チェックボックスグリッドで複数馬選択、点数自動計算
    - フォーメーション: 着順別チェックボックスグリッド、重複除外した組合せ計算
    - 流し: 軸馬選択 + 相手馬チェックボックス、1頭軸/2頭軸切替、軸馬位置指定
    - 買い目一覧プレビュー（10点以下は自動展開、超はトグル）
    - 100点超の大量買い警告表示
    - サマリー欄を「合計点数」ベースに更新
    - 保存時に組合せ展開→個別BetRecord保存
  - Playwright E2Eテスト（15テスト）: 基本表示/買い方ピル/ボックス/フォーメーション/流し/スロット操作/レスポンシブ

### Changed
- **レスポンシブ対応** — スマートフォンでの閲覧に対応
